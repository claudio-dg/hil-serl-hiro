...
log_period: int = 100
random_steps = 0
buffer_period = 500 # aumento? (originale 2000) se aumento salvo meno volte e ho meno volte il lag, ma magari lag + importante? se diminuisco magari salvo + volte ma lag meno importante? probo a diminuire
...
def main(_):
    ...
    env = Real_UR_Unscrewing_RosEnv(start_camera=FLAGS.actor) ##### attiva camere solo nel caso di ACTOR 
    # add wrappers
    ...
    if  FLAGS.actor:
            ...
            def reward_func(obs, info):
                ...
                if (info["is_inserted_enough"]):                    
                    print_green(f"Y FORCE < -1.0 = {info["is_inserted_enough"]}")
                else:
