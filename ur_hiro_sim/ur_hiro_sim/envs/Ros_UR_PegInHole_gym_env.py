...
class URPegInHoleRosEnv(MujocoGymEnv):
    ...
    def __init__(
        ...
        time_limit: float = 160.0, # metttere 30/40 per training?
        ...
        self.action_space = spaces.Box(
            ...
            low=np.asarray([-1.0, -1.0, -1.0]), # x y z traslation = 3
    ...
    def compute_observation(self) -> dict:
        ...
        tcp_position[2] -= 0.0625 # Offset lungo l'asse Z per il reale TCP (teoria 0.145)
    ...
    def _compute_reward(self) -> float:
        ...
        # tcp_desired_height = ???# hard code VEDERE CHE VALORE
        ...
        tcp_position[2] -= 0.0625 #0.135  # Offset lungo l'asse Z per il reale TCP ### 0.0625 ?

        
        dist = np.linalg.norm(object_position - tcp_position)
        # print(f" \n ##### Distance between HOLE and tcp: {dist}")
        ...
        target_distance = 0.054 #######0.0075 #0.141 # minimum desired distance at which rew = 1
        ...
        # HA SENSO QUA AGGIUNGERE UN REWARD PER L'ALTEZZA ASSOLUTA DEL TCP? FORSE NO... BOH
        # lift_reward = (object_position[2]- obj_Z_init) / (obj_Z_desired - obj_Z_init)
        # lift_reward = np.clip(lift_reward, 0.0, 1.0) # cap between 0-1
        #### NOTA: In caso, devo salvare la posa tcp iniziale? da implementare
        # abs_height_reward = (tcp_position[2]- obj_Z_init) / (obj_Z_desired - obj_Z_init)
        # abs_height_reward = np.clip(lift_reward, 0.0, 1.0) # cap between 0-1

        # total_reward = 0.3 * closeness_reward + 0.7 * lift_reward
        total_reward = closeness_reward
